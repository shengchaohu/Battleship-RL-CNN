...
class Environment():
    def __init__(self, dim, ships, name, given_board=None):
        ...
        self.num_sunk = 0
        self.ship_coords = {}
        # self.state = np.zeros([len(ships)+1, dim, dim], dtype='float32') # 1,-1,0
        self.state = np.zeros([dim, dim], dtype='float32')
        self.placement = np.zeros([len(ships), dim, dim], dtype=int)
        self.open_locations = np.full([dim, dim], 1, 'float32') # can be placed:1, else:0
        self.ground_truth = np.zeros([dim, dim], dtype='float32') # basically the same as placement
        self.done = False
    ...
    def save_sample(self,num):
        samples=[]
        env_samples=[]
        for _ in range(num):
            self.reset()
            samples.append(self.ground_truth)
            env_samples.append(copy.deepcopy(self))

        np.save('sample.npy', np.array(samples))
        with open('env_sample.pkl','wb') as f:
            # pickle.dump(str(env_samples), f)
            pickle.dump(env_samples, f)
    ...
    def step(self, guess):
        ...
        guess是我采取的行动，即一个坐标对，我要在这里check它们是击中了还是miss了
        ...
        # update guesses
        if self.state[x,y] == 0:
            self.state[x,y] = -1 # by default assume it's a miss, unless we update it later
